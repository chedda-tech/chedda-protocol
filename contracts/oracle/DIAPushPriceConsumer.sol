...
import {IDIAPushOracleReceiver, ResponseData} from "./IDIAPushOracleReceiver.sol";
...
contract DIAPriceFeed is IPriceFeed {
    ...
    function readPrice(address token) external view returns (int256 price, uint256 lastUpdated) {
        ...
        ResponseData memory data = IDIAPushOracleReceiver(oracle).updates(key);
        return (data.data.toInt256(), uint256(data.timestamp));
    }
}
